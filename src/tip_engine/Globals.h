...
inline std::function<bool()> g_RexOverlayOpenFn;

inline bool IsRexOverlayOpen() {
    return g_RexOverlayOpenFn && g_RexOverlayOpenFn();
}

inline bool g_RetipOwnUiMode = false;
inline bool g_InRomanceMinigame = false;
inline bool g_QuitConfirmActive = false;
inline bool g_StarMenuOpen = false;
...
inline bool g_FreeCamActive = false;

inline bool g_FreeCamLockView = false;

inline float g_camYawRad = 0.0f;
inline float g_camPitchRad = 0.0f;
inline uint32_t g_cameraAddr = 0;
