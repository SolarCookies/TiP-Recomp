...
{
    float sc = xe_texture15_2d_u.Sample(xe_sampler15_ppp, float3(i.t3.xy, 0.0f)).x;
    float sR = xe_texture15_2d_u.Sample(xe_sampler15_ppp, float3(i.t3.xy, 0.0f), int2( 1, 0)).x;
    float sL = xe_texture15_2d_u.Sample(xe_sampler15_ppp, float3(i.t3.xy, 0.0f), int2(-1, 0)).x;
    float sD = xe_texture15_2d_u.Sample(xe_sampler15_ppp, float3(i.t3.xy, 0.0f), int2( 0, 1)).x;
    float sU = xe_texture15_2d_u.Sample(xe_sampler15_ppp, float3(i.t3.xy, 0.0f), int2( 0, -1)).x;
    float refDepth = saturate(dot(i.t2.xyz, C_SHADOW.xyz) + C_SHADOW.w);
    ...
    float2 nm = xe_texture2_2d_u.Sample(xe_sampler2_fff, float3(i.t1.xy, 0.0f)).xy;
    nm = nm * C_NDECODE.x + C_NDECODE.y;
    float nz = sqrt(abs(-dot(nm, nm) - C_NDECODE.y));
    float3 tsN = float3(nm, nz);
    float3 N = normalize(float3(dot(i.t5.xyz, tsN), dot(i.t6.xyz, tsN), dot(i.t4.xyz, tsN)));
    float3 V = normalize(i.t2.xyz - C_CAMPOS.xyz);
    float3 L = C_LDIR.xyz;
    float NdotL = saturate(dot(-L, N));
    float3 Rlight = 2.0f * dot(N, L) * N - L;
    float RdotV = saturate(dot(Rlight, V));
    float3 diffuse = NdotL * C_LIGHT.xyz * shadow;
    float specTerm = pow(abs(RdotV), i.t2.w);
    ...
    float4 tex1 = xe_texture1_2d_u.Sample(xe_sampler1_fff, float3(i.t0.wz, 0.0f));
