...
Texture2DArray<float4> xe_texture2_2d_u : register(t9);
...
SamplerState xe_sampler2_fff : register(s4);
...
float4 main(PSInput i) : SV_Target0
{
    ...
    float3 rim = xe_texture2_2d_u.Sample(xe_sampler2_fff, float3(facing, facing, 0.0f)).xyz;
