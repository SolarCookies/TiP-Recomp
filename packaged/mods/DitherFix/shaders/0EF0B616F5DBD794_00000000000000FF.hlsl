...
TextureCube<float4> xe_texture2_cube_u : register(t5);
Texture2DArray<float4> xe_texture1_2d_u : register(t7);
...
SamplerState xe_sampler1_fff : register(s3);
...
float4 main(PSInput i) : SV_Target0
{
    ...
    float3 baseColor = i.t6.xyz;
    float  outAlpha  = i.t6.w;
    float2 nm = xe_texture0_2d_u.Sample(xe_sampler0_fff, float3(i.t0.xy, 0.0f)).xy;
    nm = nm * C_NDECODE.x + C_NDECODE.y;
    float  nz = sqrt(abs(-dot(nm, nm) - C_NDECODE.y));
    float3 tsN = float3(nm, nz);
    float3 N = float3(dot(i.t4.xyz, tsN), dot(i.t5.xyz, tsN), dot(i.t3.xyz, tsN));
    N = normalize(N);
    ...
    if ((xe_bool_constants[1].x & 0x2u) != 0u) {
        ...
        baseColor += xe_texture2_cube_u.Sample(xe_sampler2_fff, reflVec).xyz;
    }
    float3 ambient  = max(C_AMBIENT.xyz, 0.0f);   // fix
    float3 color = baseColor * (diffuse + ambient) + specular;
    float  facing = saturate(dot(N, -V));
    float3 rim    = xe_texture1_2d_u.Sample(xe_sampler1_fff, float3(facing, facing, 0.0f)).xyz;
