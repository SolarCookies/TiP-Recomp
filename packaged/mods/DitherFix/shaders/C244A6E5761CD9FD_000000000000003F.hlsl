...
#define C_ALBBIAS xe_float_constants[9]
#define C_ALBSCALE xe_float_constants[10]

Texture2DArray<float4> xe_texture15_2d_u : register(t1);
TextureCube<float4> xe_texture1_cube_u : register(t3);
Texture2DArray<float4> xe_texture0_2d_u : register(t5);

SamplerState xe_sampler15_ppp : register(s0);
SamplerState xe_sampler1_fff : register(s1);
SamplerState xe_sampler0_fff : register(s2);
...
float4 main(PSInput i) : SV_Target0
{
    ...
    float3 baseColor = i.t4.xyz * C_ALBSCALE.xyz + C_ALBBIAS.xyz;
    float outAlpha = i.t4.w;
    float3 N = normalize(i.t3.xyz);
    ...
    if ((xe_bool_constants[1].x & 0x2u) != 0u) {
        ...
        baseColor += xe_texture1_cube_u.Sample(xe_sampler1_fff, reflVec).xyz;
    }
    float4 tex0 = xe_texture0_2d_u.Sample(xe_sampler0_fff, float3(i.t0.xy, 0.0f));
