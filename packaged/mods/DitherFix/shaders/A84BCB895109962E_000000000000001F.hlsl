...
float4 main(PSInput i) : SV_Target0
{
    ...
    float3 albedo = albSample.xyz * C_ALBSCALE.xyz + C_ALBBIAS.xyz;
    ...
    if ((xe_bool_constants[1].x & 0x2u) != 0u) {
        ...
        float3 cube = xe_texture2_cube_u.Sample(xe_sampler2_fff, reflVec).xyz;
        albedo += cube;
    }
    ...
    float3 color = albedo * lighting + specular;
    color *= tex1.xyz;
    color *= C_OUTMUL.xyz;
