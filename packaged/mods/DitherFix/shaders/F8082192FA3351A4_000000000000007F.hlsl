...
TextureCube<float4> xe_texture2_cube_u : register(t7);
...
float4 main(PSInput i) : SV_Target0
{
    ...
    float2 nm = xe_texture1_2d_u.Sample(xe_sampler1_fff, float3(i.t0.xy, 0.0f)).xy;
    ...
    if ((xe_bool_constants[1].x & 0x2u) != 0u) {
        ...
        albedo += xe_texture2_cube_u.Sample(xe_sampler2_fff, reflVec).xyz;
    }
