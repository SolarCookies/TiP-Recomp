...
TextureCube<float4> xe_texture4_cube_u : register(t9);
...
SamplerState xe_sampler4_fff : register(s4);
...
float4 main(PSInput i) : SV_Target0
{
    ...
    if ((xe_bool_constants[1].x & 0x2u) != 0u) {
        ...
        albedo += xe_texture4_cube_u.Sample(xe_sampler4_fff, reflVec).xyz;
    }
    float3 ambient = max(C_AMBIENT.xyz, 0.0f);
    float3 color = albedo * (diffuse + ambient) + specular;
    float facing = saturate(dot(N, -V));
    float3 rim = xe_texture3_2d_u.Sample(xe_sampler3_fff, float3(facing, facing, 0.0f)).xyz;
