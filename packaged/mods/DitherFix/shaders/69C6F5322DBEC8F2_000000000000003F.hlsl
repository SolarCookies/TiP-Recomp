...
Texture2DArray<float4> xe_texture1_2d_u : register(t5);
Texture2DArray<float4> xe_texture2_2d_u : register(t7);
TextureCube<float4> xe_texture5_cube_u : register(t9);
Texture2DArray<float4> xe_texture3_2d_u : register(t11);
Texture2DArray<float4> xe_texture4_2d_u : register(t13);

SamplerState xe_sampler15_ppp : register(s0);
SamplerState xe_sampler0_fff : register(s1);
SamplerState xe_sampler1_fff : register(s2);
SamplerState xe_sampler2_fff : register(s3);
SamplerState xe_sampler5_fff : register(s4);
SamplerState xe_sampler3_fff : register(s5);
SamplerState xe_sampler4_fff : register(s6);
...
float4 main(PSInput i) : SV_Target0
{
    ...
    float3 specMask = xe_texture1_2d_u.Sample(xe_sampler1_fff, float3(i.t0.xy, 0.0f)).xyz;
    ...
    specular *= specMask * i.t3.w;
    if ((xe_bool_constants[1].x & 0x2u) != 0u) {
        float3 reflVec = 2.0f * dot(N, V) * N - V;
        float3 cube = xe_texture5_cube_u.Sample(xe_sampler5_fff, reflVec).xyz;
        float3 reflMask = xe_texture3_2d_u.Sample(xe_sampler3_fff, float3(i.t0.xy, 0.0f)).xyz;
        albedo += cube * reflMask;
    }
    float3 ambient = max(C_AMBIENT.xyz, 0.0f);
    float3 color = albedo * (diffuse + ambient) + specular;
    float facing = saturate(dot(N, -V));
    float3 rim = xe_texture4_2d_u.Sample(xe_sampler4_fff, float3(facing, facing, 0.0f)).xyz;
    color += rim * C_LIGHT.xyz * shadow;
