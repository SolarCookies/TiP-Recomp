...
TextureCube<float4> xe_texture1_cube_u : register(t5);
...
float4 main(PSInput i) : SV_Target0
{
    ...
    float4 baseSample = xe_texture0_2d_u.Sample(xe_sampler0_fff, float3(i.t0.xy, 0.0f));
    float3 baseColor = baseSample.xyz;
    float outAlpha = baseSample.w;
